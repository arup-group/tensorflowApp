...
const App = () => {
  ...
  const [imageLink,setImageLink] = useState("https://upload.wikimedia.org/wikipedia/commons/5/5c/Taylor_Swift_%286820711660%29.jpg")
  const [pickedImage, setPickedImage] = useState("");
  const [faces,setFaces]=useState([])
  const [faceDetector,setFaceDetector]=useState("")
  const [maskDetector,setMaskDetector]=useState("")
  const [maskModel, setMaskModel] = useState(false);
  ...
  //     // Load mobilenet.
  //     await tf.ready();
  //     const modelJson = await require("./assets/model2/model.json");
  //     const modelWeight = await require("./assets/model2/group1-shard.bin");
  ...
  return (
    ...
    {maskModel == true && <Button 
        title="Predict"
        onPress={()=>{getFaces()}}
        disabled={!isEnabled}
      />}
      {maskModel == faceDetector &&<Text>model is not ready yet</Text>}
      {maskModel == true &&<Text>you can click "Predict" now</Text>}
  </View>
    // <View
    //   style={{
    //     height: '100%',
